...
ride_feedback AS (
    ...
    WHERE lower(r.ride_status) != 'cancelled'
      AND f.captain_rating IS NOT NULL
...
SELECT
    ...
    u.signup_date,
    fr.first_ride_date,
    MAX(r.ride_date) FILTER (WHERE lower(r.ride_status) != 'cancelled') AS last_ride_date,
    COUNT(r.ride_id) FILTER (WHERE lower(r.ride_status) != 'cancelled') AS total_rides,
    COALESCE(SUM(rp.total_payment), 0) AS total_revenue,
    CASE WHEN COUNT(rp.total_payment) > 0 THEN SUM(rp.total_payment)/COUNT(rp.total_payment) ELSE NULL END AS avg_revenue_per_ride,
    COUNT(r.ride_id) FILTER (WHERE r.ride_date >= CURRENT_DATE - INTERVAL '30 days' AND lower(r.ride_status) != 'cancelled') AS booking_frequency,
    CASE WHEN COUNT(r.ride_id) FILTER (WHERE lower(r.ride_status) != 'cancelled') > 0 THEN 1 ELSE 0 END AS is_active,
    rf.avg_captain_rating,
    rf.most_frequent_issue
FROM silver.users u
LEFT JOIN silver.rides r ON u.user_id = r.user_id
LEFT JOIN ride_payment rp ON r.ride_id = rp.ride_id
LEFT JOIN ride_feedback rf ON u.user_id = rf.user_id
LEFT JOIN first_ride fr ON u.user_id = fr.user_id
GROUP BY u.user_id, u.name, u.age, u.gender, u.city, u.signup_date, fr.first_ride_date, rf.avg_captain_rating, rf.most_frequent_issue;
"""
...
SILVER_QUERIES = {
    "total_rides": """
                   ...
                   FROM silver.rides
                   WHERE lower(ride_status) != 'cancelled';
                   """,
    "total_revenue": """
    ...
    "booking_frequency": """
                         ...
                         WHERE r.ride_date >= CURRENT_DATE - INTERVAL '30 days'
                           AND lower(r.ride_status) != 'cancelled';
                         """,
    "is_active": """
...
def create_or_replace_gold_user_aggregate():
    log_message("Dropping and recreating gold.user_aggregate table...")
    with engine.connect() as conn:
        conn.execute(text(GOLD_USER_AGGREGATE_SQL))
        conn.commit()
    log_message("gold.user_aggregate table created/updated successfully.")

# -----------------------
# Reconciliation function
# -----------------------
def reconcile_silver_gold():
    log_message("Starting reconciliation...")
    results = []
